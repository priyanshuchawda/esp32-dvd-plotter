...
// Single-page drawing UI served by the ESP32. __BED__ is substituted with the
// real bed size at request time so the page and firmware cannot disagree.
...
<script>
const BED=__BED__,c=document.getElementById('c'),x=c.getContext('2d'),st=document.getElementById('st');
let strokes=[],cur=null,busy=false;

function fit(){const s=Math.min(innerWidth-24,innerHeight-190);c.width=c.height=s;draw()}
function draw(){
...
// Ramer-Douglas-Peucker: a finger drag makes hundreds of near-collinear points
// and every one of them would become a separate G1 the board has to chew on.
function rdp(p,eps){
...
function gcode(){
 const k=BED/c.width,out=['G21','G90','M300 S50'];
 // Canvas Y grows downward, the plotter's grows upward.
 const m=p=>[(p[0]*k).toFixed(2),((c.height-p[1])*k).toFixed(2)];
